...
if($check !=[])
{
	foreach($check as $row)
	{
        switch($row->getStatus())
            {
                ...
                case 0 : $type = '<span class="las la-exclamation icon">';
                         $name = 'Waiting';
                         $action = '
                                    <button   type="button" class="btn_edit btn" id="'.$row->getCusid().'"  name="processing"> 
                                    <span class="las la-ellipsis-h"></span>
                                    </button>
                                    <button   type="button" class="btn_edit btn" id="'.$row->getCusid().'"  name="done" >
                                    <span class="las la-check"></span>
                                    </button>';
                         break;
                case 1 : $type = '<span class="las la-ellipsis-h icon"></span>';
                         $name = 'Processing';
                         $action = '<button   type="button" class="btn_edit btn" id="'.$row->getCusid().'"  name="waiting" > 
        ...
        if($type1!=="C")
        {
            $action .='<button   type="button" class="btn_delete_task btn" id="'.$row->getCusid().'"  name="delete_task"  >
