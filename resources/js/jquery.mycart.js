  ...
  var loadMyCartEvent = function (targetSelector) {
    ...
    if (!$("#" + idCartModal).length) {
      $('body').append(
          ...
          '<h4 class="modal-title" id="myModalLabel"><span class="glyphicon glyphicon-shopping-cart"></span>ĐƠN HÀNG</h4>' +
          '</div>' +
          '<div class="modal-body">' +
          '<table class="table table-hover table-responsive" id="' + idCartTable + '"></table>' +
          '</div>' +
          '<div class="modal-footer">' +
          '<button type="button"  id="btn-form" name="btn-form"  data-dismiss="modal" aria-label="Close" onclick="formdisplay()" class="btn btn-primary btn-form-display  " >THANH TOÁN</button>' +
          '</div>' +
          '</div>' +
          '</div>' +
          '</div>'
    ...
    var drawTable = function () {
      ...
      $.each(products, function () {
        ...
        $cartTable.append(
            '<tr title="'  + '" data-id="' + this.id + '" data-price="' + this.price + '">' +
      ...
      $cartTable.append(products.length ?
          ...
          '<td></td>' +
          '</tr>' :
          '<div class="alert alert-danger" role="alert" id="' + idEmptyCartMessage + '">Chưa có sản phẩm</div>'
      );
